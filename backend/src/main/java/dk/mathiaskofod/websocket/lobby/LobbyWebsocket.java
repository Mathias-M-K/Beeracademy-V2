...
public class LobbyWebsocket {
    ...
    public void onError(RuntimeException e){
        ...
        if(e instanceof SessionNotFoundException){
            connection.closeAndAwait(new CloseReason(WebsocketCode.SESSION_NOT_FOUND.getCode()));
            return;
        }

        if (e instanceof LobbyNotFoundException) {
            connection.closeAndAwait(new CloseReason(WebsocketCode.LOBBY_NOT_FOUND.getCode()));
        }
    }
