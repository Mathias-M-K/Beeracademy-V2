...
@Component({
  ...
  imports: [FormField, DotLoader, ExistingParticipant],
...
export class JoinPage {
  ...
  protected getLobbyParticipantToken(participantName: string) {
    ...
    this.lobbyApi
      .fetchParticipantToken(this.partyId(), participantName)
      .pipe(
      ...
      .subscribe({
        ...
        error: () =>
          this.toastService.showToast('Der skete en fejl', 'Kunne ikke deltage i lobbyen', 'error'),
      });
  }
  ...
  protected connectAsParticipant(participant: PartyParticipantDto) {
    ...
    this.gameApi
      .getGamePlayerToken(this.partyInfo().id, participant.id)
      .pipe(
        takeUntilDestroyed(this.destroyRef),
        finalize(() => this.joining.set(false)),
        timeout({ each: 8000 }),
  ...
  protected requestRelease(participant: PartyParticipantDto) {
    ...
    this.gameApi
      .requestPlayerRelease(this.partyId(), participant.id)
      .pipe(takeUntilDestroyed(this.destroyRef), timeout({ each: 8000 }))
      .subscribe({
        next: () => {
