...
export class GameService implements PartyContextProvider {
  ...
  private handleWebsocketConnectionDroppedWithError(error?: unknown) {
    ...
    console.debug(
      'Lost connection to game-websocket. Message: ',
      errorObj?.message,
      ', code: ',
      errorObj?.cause,
    );
    ...
    switch (errorObj.cause) {
      ...
      case WebsocketCode.TryAgainLater: {
        const visibilityState = document.visibilityState;
        console.warn(
          'Transient disconnect, awaiting resume. Code:',
          errorObj.cause,
          ', Page visible:',
          visibilityState,
        );
  ...
  private handleWebsocketMessage(msg: WebsocketEnvelope) {
    const supportedEventCategories: string[] = [
      'GAME_EVENT',
      'GAME_CLIENT_EVENT',
      'PLAYER_CLIENT_EVENT',
    ];
  ...
  private handlePlayerConnected(event: GameEventEnvelope) {
    ...
    if (this.releaseRequests().includes(playerConnectedEvent.playerId)) {
      this._releaseRequests.update((playerIds) =>
        playerIds.filter((playerId) => playerId !== playerConnectedEvent.playerId),
      );
    }
  ...
  private handlePlayerReleaseRequested(event: GameEventEnvelope) {
    const playerReleaseRequestedEvent: PlayerReleaseRequestedEvent =
      event.payload as PlayerReleaseRequestedEvent;
  ...
  public dispatchReleaseAction(playerId: string) {
    if (this.releaseRequests().includes(playerId)) {
      this._releaseRequests.update((playerIds) =>
        playerIds.filter((requestPlayerId) => requestPlayerId !== playerId),
      );
    }
  ...
  private updatePlayerConnectionStatus(
    playerId: string,
    connected: boolean,
    claimed: boolean,
  ): void {
  ...
  private openPausePanel() {
    ...
    const registeredPausesSummed =
      gameTimeReport.pauses?.reduce((accumulated, current) => accumulated + current, 0) ?? 0;
  ...
  private openChugOverlay() {
    ...
    this.chugOverlay = this.overlayService.openOverlay<number>({
