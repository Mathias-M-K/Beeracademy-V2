...
export class LobbyApi {
  ...
  public createLobby(lobbyName: string): Observable<CreateLobbyResponse> {
    ...
    return this.httpClient.post<CreateLobbyResponse>(
      this.apiUrl + '/lobbies',
      null,
      requestOptions,
    );
  ...
  public fetchParticipantToken(
    partyId: string,
    participantName: string,
  ): Observable<RegisterPlayerResponse> {
    const requestOptions = {
      params: new HttpParams().set('participantName', participantName),
      withCredentials: true,
    };

    return this.httpClient.post<RegisterPlayerResponse>(
      this.apiUrl + `/lobbies/${partyId}/register`,
      null,
      requestOptions,
    );
  }
