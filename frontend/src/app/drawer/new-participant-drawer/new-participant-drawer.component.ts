...
@Component({
  imports: [DrawerDefaultHeaderComponent, CdkTrapFocus, ReactiveFormsModule, FormField],
...
export class NewParticipantDrawerComponent {
  ...
  protected readonly newParticipantForm = form(this.newParticipantModel, (model) => {
    ...
    uniqueNameValidator(model.participantName, () =>
      this.lobbyService.participants().map((participant) => participant.name ?? ''),
    );
  });
